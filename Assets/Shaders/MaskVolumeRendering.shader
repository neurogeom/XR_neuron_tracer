Shader "Unlit/MaskedVolumeRendering"
{
    Properties
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                for(int t = 0; t< 500000; t ++){
                    ...
                    float val = max(tex3Dlod(_Volume, uv)-0.2f,0);
                    float mask_val = tex3Dlod(_Mask, uv);
                    if(mask_val>0.01) {
                        col.x = lerp(1,0,mask_val);
                        col.y = lerp(0,1,mask_val);
                    }
                    else {
                        //col = float3(max(col.x,val),max(col.y,val),max(col.z,val));
                    }
                    alpha = max(alpha,val);
                    //col = (1 - alpha)*val + col;
